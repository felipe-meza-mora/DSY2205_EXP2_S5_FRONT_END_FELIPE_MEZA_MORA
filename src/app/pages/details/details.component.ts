...
export class DetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (productId) {
      ...
      this.productService.getProductById(numericProductId.toString()).subscribe(response => {
        console.log(response); // Verifica que la respuesta sea la esperada
        this.product = response.product; // Accede a la clave 'product'
      });
    }
  }
