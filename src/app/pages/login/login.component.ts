...
export class LoginComponent implements OnInit {
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      const usuario = await this.usersService.validateUser(email, password).toPromise(); // Asegúrate de que validateUser retorne un Observable<User>
      if (usuario) {
        // Guardar los datos del usuario en localStorage
        const userData: User = {
          id: usuario.id,
          nombre: usuario.nombre,
          password: usuario.password, // Considera si necesitas esto en el frontend por razones de seguridad
